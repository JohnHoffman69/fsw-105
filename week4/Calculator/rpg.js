...
let demons = [`Demons`, `Red Demon`, `Black Demon`, `Yellow Demon`];
let approachingDemon = demons[Math.floor(Math.random() * demons.length)];
let demonAttack = Math.floor(Math.random() * 30);
let rewards = [
  "Demon Repellent",
  "Heavens Blade",
  "Angelic Horn",
...
let player = readlineSync.question(`Enter name to begin game: `);
console.log(`\n\nWelcome ${player}! You have chosen to play my Demons RPG. 
The mission is to walk through the bowles of Hell and kill a demon. Start your mission if you dare!`);
...
const demonRpg = () => {
  ...
  if (playerOptions[option] == `Exit`) {
    ...
    let key = Math.random();
    if (key <= 0.25) {
      console.log(`No Demons around.`);
    } else if (key >= 0.25) {
      console.log(`${approachingDemon}! You Must DIE!\n`);
        ...
        switch (runOrAttack) {
          ...
          case "a":
            ...
            if (demonsHealth <= 0) {
              ...
              console.log(`You have both died in battle. GAME OVER!`);
            }
        }
      }
    }
